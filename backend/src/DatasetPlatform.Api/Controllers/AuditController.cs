...
public sealed class AuditController(IDatasetService datasetService, IRequestUserContextAccessor userContextAccessor) : ControllerBase
{
    ...
    /// </param>
    [HttpGet]
    public async Task<ActionResult<IReadOnlyList<AuditEvent>>> GetAudit([FromQuery] string? datasetKey, CancellationToken cancellationToken)
    {
        try
        {
            var records = await datasetService.GetAuditAsync(userContextAccessor.GetCurrent(), cancellationToken, datasetKey);
            return Ok(records);
        }
